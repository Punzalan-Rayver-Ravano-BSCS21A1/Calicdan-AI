...
<<<<<<< HEAD
      ...
      sessions = sessions.map(s => {
        if (!s.title && s.messages && s.messages.length > 0) {
          ...
          if (firstUserMsg) s.title = firstUserMsg.content.slice(0, 40);
        }
        return s;
      });
...
async function startNewChat() {
  ...
  // Archive current session first
  if (currentSessionId && messages.length > 0) {
    await archiveCurrentConversationBatch(currentSessionId, messages);
  }
  ...
  try {
    ...
    // ✅ Update thread history, remove old duplicates
    let threads = JSON.parse(localStorage.getItem(threadKey) || "[]");
    threads = threads.filter(t => t.id !== currentSessionId);
    threads.unshift({
      id: currentSessionId,
      ...
      endedAt: Date.now(),
      badge: "Draft",
      title: messages.find(m => m.sender === "user")?.content.slice(0, 40) || "New conversation"
    });
    localStorage.setItem(threadKey, JSON.stringify(threads));

    // ✅ Update chat header immediately (badge will auto-calc as Draft)
    updateChatHeader(currentSessionId, messages);
...
/**
 * Batch-archive all messages in a session
 */
async function archiveCurrentConversationBatch(sessionId, allMessages) {
  try {
    const session = getSession();
    if (!session || !session.session_token || !sessionId) return false;

    // Only include unarchived messages
    const newMessages = allMessages.filter(m => !m.archived);
    if (newMessages.length === 0) return true; // nothing to archive

    // Prepare payload
    const payload = {
      session_token: session.session_token, // <--- MISSING
      session_id: sessionId,
      messages: newMessages
        .map(m => {
          const msg = {};
          if (m.content && m.content.trim()) msg.message = m.content;
          msg.sender = m.sender === "assistant" ? "ai" : "user"; // must match Pydantic
          if (m.attachmentUrl) msg.attachmentUrl = m.attachmentUrl;
          if (m.attachmentName) msg.attachmentName = m.attachmentName;
          return msg;
        })
        .filter(m => m.message || m.attachmentUrl)
    };


    if (payload.messages.length === 0) return true; // nothing to send

    console.log("Archiving batch payload:", payload);

    const resp = await fetch(`${window.AuthModule.BACKEND_URL}/chat/batch`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${session.session_token}`
    ...
    if (!resp.ok) {
      ...
      console.warn("⚠️ Archive batch failed:", data.detail || resp.statusText);
      return false;
    }

    // mark as archived locally
    newMessages.forEach(m => m.archived = true);

    // update header badge
    updateChatHeader(sessionId, allMessages);

    console.log(`All ${newMessages.length} new messages archived successfully ✅`);
    return true;
...
// ---- Update chat header title and status ----
function updateChatHeader(sessionId, messages = [], options = {}) {
...
if (sessionId) {
    // Get title from threads or first user message
    const userId = getUserIdForHistory();
    const threadsKey = `chatThreads_user_${userId}`;
    ...
    if (userMessages.length === 0) badge = "Draft";              // no user messages yet
    else if (userMessages.every(m => m.archived)) badge = "Archived"; 
    else badge = "Active";                                       
} else {
    // sessionId is null → new chat
    badge = "Draft"; // always Draft for new chat
}
...
function selectHistorySession(sessionId) {
  const userId = getUserIdForHistory();
  const threads = JSON.parse(localStorage.getItem(`chatThreads_user_${userId}`) || "[]");
  const thread = threads.find(t => t.id === sessionId);
  if (!thread) return;

  messages = thread.messages.map(m => ({
    ...m,
    archived: typeof m.archived === "boolean" ? m.archived : m.sender === "assistant" ? true : false
  }));
  ...
  // Save active session
  localStorage.setItem(`calicdan-activeSession_user_${userId}`, JSON.stringify({
    sessionId: currentSessionId,
    messages
  }));

  renderMessages(true);
  updateChatHeader(currentSessionId, messages); // ✅ badge will now be correct
  saveThreadStatus(currentSessionId, messages);
}
...
>>>>>>> 8c70d7e746ab369a6c9a727f7e10e26425ff846e
  const userMessage = {
    ...
    sender: "user",
    timestamp: new Date().toISOString(),
    archived: false // do NOT mark as archived immediately
  };
...
<<<<<<< HEAD
    ...
    // ✅ Update chat header and save thread status
    updateChatHeader(currentSessionId, messages);
    saveThreadStatus(currentSessionId, messages);
...
function saveThreadStatus(sessionId, messages) {
  ...
  let threads = JSON.parse(localStorage.getItem(threadKey) || "[]");
  threads = threads.filter(t => t.id !== sessionId);

  // Determine badge
  const userMessages = messages.filter(m => m.sender === "user");
  let badge;
  if (userMessages.length === 0) badge = "Draft";                   
  else if (userMessages.every(m => m.archived)) badge = "Archived"; 
  else badge = "Active";                                             

  // --- NEW: Derive title from first user message ---
  let title = null;
